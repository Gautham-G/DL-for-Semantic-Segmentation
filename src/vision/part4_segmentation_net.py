...
class SimpleSegmentationNet(nn.Module):
    ...
    def forward(
        ...
        yhat = torch.argmax(logits, dim = 1).float()

        # print(logits.shape, yhat.shape, "shapes of log, y, y'")
        # main_loss = self.criterion(yhat, y.long())
        if y is None:
            main_loss = None
            aux_loss = None
        else:
            main_loss = self.criterion(logits, y.long())
