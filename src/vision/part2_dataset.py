...
def make_dataset(split: str, data_root: str, data_list_fpath: str) -> List[Tuple[str, str]]:
    ...
    file = open(data_list_fpath)
    list_from_txt_file = []

    for f in file:
        f = f.strip('\n')
        list_from_txt_file.append(f.split(' '))

    for paths in list_from_txt_file:
        add_tuple = (data_root + '/' + paths[0], data_root + '/' + paths[1])
        # print(data_root, paths[0], paths[1])
        # print(add_tuple)
        image_label_list.append(add_tuple)
