...
class PPM(nn.Module):
    ...
    def __init__(self, in_dim: int, reduction_dim: int, bins: List[int]) -> None:
        ...
        for n in bins:
            ...
            seq_module = nn.Sequential(
                nn.AdaptiveAvgPool2d(n),
                nn.Conv2d(in_channels = in_dim, out_channels = reduction_dim, kernel_size = (1,1), bias = set_bias),
                nn.BatchNorm2d(reduction_dim, affine = False),
                nn.ReLU()
            )
            self.features.append(seq_module)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        N, in_dim, H_in, W_in = x.shape
        # print(x.shape)
        channel_list = []
        channel_list.append(x)
        # output = x
        for modules in self.features:
            x_new = modules(x)
            _, reduction_dim, H_out, W_out = x_new.shape
            # print(H_in, H_out)
            upsampling = nn.Upsample(size = (H_in, W_in), mode = 'bilinear', align_corners = True)
            upsampled_output = upsampling(x_new)
            channel_list.append(upsampled_output)
            # print(upsampled_output, '\n')
  
        # channel_list.reverse()
        output = torch.cat(channel_list, dim = 1)
        # print(output.shape)
