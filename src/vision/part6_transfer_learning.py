...
def model_and_optimizer(args, model) -> Tuple[nn.Module, torch.optim.Optimizer]:
    ...
    model = PSPNet(layers = int(args.layers), num_classes=int(2), pretrained=bool(args.pretrained), zoom_factor=int(args.zoom_factor))
    momentum = float(args.momentum)
    weight_decay = float(args.weight_decay)
    lr = float(args.base_lr)

    optimizer = torch.optim.SGD([
            {'params': model.layer0.parameters(), 'lr': lr},
            ...
            {'params': model.cls.parameters(), 'lr': 10*lr},
            {'params': model.ppm.parameters(), 'lr': 10*lr},
